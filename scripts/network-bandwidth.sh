...
format_speed() {
  ...
  #echo $(echo "scale=2; $1/125000" | bc -l) "Mbps"
  printf '%.2f Mbps\n' "$(echo "scale=2; $1/125000" | bc)"
}
...
main() {
  ...
  if [ -z "$old_value" ]; then
    ...
    $(set_tmux_option "@network-bandwidth-previous-value" "D: $(format_speed $download_speed) • U: $(format_speed $upload_speed)")
  fi
